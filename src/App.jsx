    ...
    <div className="app">
      ...
      <main className="main">
        ...
        <div className="topbar">
          ...
          <div className="topbar-actions">
            {B.challenge && ['dashboard','dias','estatisticas'].includes(B.screen) && (
              <button className="btn btn-ghost btn-sm" style={{ color: 'var(--accent2)', borderColor: 'rgba(79,110,247,0.35)' }} onClick={() => B.setModal({ open: true, type: 'export', retornoFinal: 0 })}>
                <Download size={13} /><span className="btn-label"> Exportar</span>
              </button>
            )}
            {B.challenge && B.screen === 'dia' && B.undoStack.length > 0 && (
              <button className="btn btn-ghost btn-sm" style={{ color: '#a78bfa', borderColor: 'rgba(167,139,250,0.35)' }} onClick={B.desfazerUltimo}>
                <RotateCcw size={13} /><span className="btn-label"> Desfazer</span>
              </button>
            )}
            {B.challenge && B.screen === 'dia' && B.diaAtual && (
              <button className="btn btn-ghost btn-sm" style={{ color: 'var(--amber)', borderColor: 'rgba(245,158,11,0.35)' }} onClick={() => B.setModal({ open: true, type: 'reset-dia', retornoFinal: 0 })}>
                <RotateCcw size={13} /><span className="btn-label"> Resetar dia</span>
              </button>
            )}
            {B.challenge && B.screen !== 'home' && (
              <button className="btn btn-danger btn-sm" onClick={() => B.setModal({ open: true, type: 'reset', retornoFinal: 0 })}>Resetar</button>
            )}
          </div>
        </div>
          ...
          <div className="screen active page">
            <div className="home-grid">
              <div className="card">
                ...
                <div className="input-group"><label>Banca inicial (R$)</label><input type="number" value={B.cfgBanca} onChange={(e) => B.setCfgBanca(e.target.value)} /></div>
                <div className="input-row cols-2">
                  <div className="input-group"><label>Duração (dias)</label><input type="number" value={B.cfgDias} onChange={(e) => B.setCfgDias(e.target.value)} /></div>
                  <div className="input-group"><label>Odd alvo/dia</label><input type="number" step="0.01" value={B.cfgOdd} onChange={(e) => B.setCfgOdd(e.target.value)} /></div>
                </div>
                <div className="meta-preview">
                  <div className="meta-preview-label">Meta final estimada</div>
                  <div className="meta-preview-value">{brl(B.cfgBanca * Math.pow(parseFloat(B.cfgOdd) || 1, parseInt(B.cfgDias) || 1))}</div>
                  <div style={{ fontSize: 11, color: 'rgba(79,110,247,.55)', marginTop: 6 }}>Se acertar todos os {B.cfgDias} dias com odd {B.cfgOdd}</div>
                </div>
                <button className="btn btn-primary" style={{ marginTop: 20 }} onClick={B.iniciarDesafio}><Play size={16} /> Iniciar Desafio</button>
              </div>
            </div>
          </div>
        )}
        ...
        {B.screen === 'dashboard' && B.challenge && (() => {
          ...
          return (
            <div className="screen active page">
              ...
              {prob && (
                <div style={{
                  marginBottom: 20, background: 'var(--bg2)', borderRadius: 16, padding: '18px 22px',
                  border: `1px solid ${!prob.prob ? 'var(--border)' : prob.prob >= 0.7 ? 'rgba(34,197,94,0.25)' : prob.prob >= 0.4 ? 'rgba(245,158,11,0.25)' : 'rgba(239,68,68,0.25)'}`,
                  ...
                  <div style={{ position: 'relative', width: 76, height: 76, flexShrink: 0 }}>
                    <svg width="76" height="76" viewBox="0 0 76 76" style={{ transform: 'rotate(-90deg)' }}>
                      ...
                      {prob.prob !== null && prob.prob > 0 && (
                        <circle cx="38" cy="38" r="30" fill="none"
                          stroke={prob.prob >= 0.7 ? 'var(--green)' : prob.prob >= 0.4 ? 'var(--amber)' : 'var(--red)'}
                          strokeWidth="7" strokeLinecap="round"
                          strokeDasharray={`${2 * Math.PI * 30}`}
                          strokeDashoffset={`${2 * Math.PI * 30 * (1 - prob.prob)}`}
                        />
                      )}
                    </svg>
                    <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
                      {prob.prob === null ? (
                        <span style={{ fontSize: 10, color: 'var(--text3)', textAlign: 'center', lineHeight: 1.3 }}>poucos<br/>dados</span>
                      ) : (
                        <>
                          <span style={{ fontSize: 18, fontWeight: 800, fontFamily: 'monospace', lineHeight: 1, color: prob.prob >= 0.7 ? 'var(--green)' : prob.prob >= 0.4 ? 'var(--amber)' : 'var(--red)' }}>{Math.round(prob.prob * 100)}%</span>
                          <span style={{ fontSize: 9, color: 'var(--text3)', marginTop: 2 }}>chance</span>
                        </>
                      )}
                    </div>
                  </div>
                  {/* info */}
                  <div>
                    <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--text)', marginBottom: 8 }}>Probabilidade de bater a meta</div>
                    {prob.prob === null ? (
                      <div style={{ fontSize: 12, color: 'var(--text3)' }}>{prob.msg}</div>
                    ) : (
                      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(80px,1fr))', gap: 12 }}>
                        {[
                          { label: 'Taxa acerto',    value: `${Math.round(prob.taxa * 100)}%` },
                          { label: 'Dias restantes', value: `${prob.restantes}` },
                          { label: 'Greens p/ meta', value: `${prob.minGreens}` },
                          { label: 'Banca projetada',value: compact(prob.bancaProjetada) },
                        ].map(({ label, value }) => (
                          <div key={label} style={{ textAlign: 'center' }}>
                            <div style={{ fontSize: 15, fontWeight: 800, fontFamily: 'monospace', color: 'var(--text)' }}>{value}</div>
                            <div style={{ fontSize: 10, color: 'var(--text3)', marginTop: 2 }}>{label}</div>
                          </div>
                        ))}
                      </div>
                    )}
                    {prob.prob === 0 && <div style={{ fontSize: 12, color: 'var(--red)', marginTop: 6, fontWeight: 600 }}>Meta inalcançável com os dias restantes.</div>}
                  </div>
                </div>
              )}
                ...
                <div className="card">
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
                    ...
                    {(B.challenge.marcos || []).length > 0 && (
                      <span style={{ fontSize: 11, color: 'var(--text3)', display: 'flex', alignItems: 'center', gap: 4 }}><Flag size={11} /> {B.challenge.marcos.length} marco(s)</span>
                    )}
                  </div>
                ...
                <div className="card">
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr auto', gap: 8, marginBottom: 16 }}>
                    <div>
                      ...
                      <input type="number" min={1} max={B.challenge.config.dias} placeholder={`1-${B.challenge.config.dias}`} value={B.novoMarcoDia} onChange={(e) => B.setNovoMarcoDia(e.target.value)} style={{ padding: '8px 10px', fontSize: 13 }} />
                    </div>
                    <div>
                      <label style={{ fontSize: 11 }}>Banca (R$)</label>
                      <input type="number" placeholder="ex: 100" value={B.novoMarcoBanca} onChange={(e) => B.setNovoMarcoBanca(e.target.value)} style={{ padding: '8px 10px', fontSize: 13 }} />
                    </div>
                    <div style={{ display: 'flex', alignItems: 'flex-end' }}>
                      <button className="btn btn-ghost btn-sm" style={{ color: 'var(--amber)', borderColor: 'rgba(245,158,11,0.3)' }} onClick={B.adicionarMarco}><Flag size={13} /> Add</button>
                    </div>
                  </div>
                  {(B.challenge.marcos || []).length === 0 ? (
                    <div style={{ fontSize: 12, color: 'var(--text3)', textAlign: 'center', padding: '20px 0' }}>Nenhuma meta intermediária definida</div>
                  ) : (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                      {B.challenge.marcos.map((m) => {
                        ...
                        return (
                          <div key={m.dia} style={{ background: atingiu ? 'rgba(34,197,94,0.07)' : passou ? 'rgba(239,68,68,0.07)' : 'var(--bg3)', border: `1px solid ${atingiu ? 'rgba(34,197,94,0.2)' : passou ? 'rgba(239,68,68,0.2)' : 'var(--border)'}`, borderRadius: 10, padding: '10px 14px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            <div>
                              <div style={{ fontSize: 12, fontWeight: 700, color: atingiu ? 'var(--green)' : passou ? 'var(--red)' : 'var(--amber)' }}>{atingiu ? '✓' : passou ? '✗' : '◎'} Dia {m.dia}</div>
                              <div style={{ fontSize: 13, fontWeight: 700, fontFamily: 'monospace', marginTop: 2 }}>{brl(m.banca)}</div>
                              {passou && <div style={{ fontSize: 11, color: 'var(--text3)', marginTop: 2 }}>Real: {brl(bancaReal)} ({bancaReal >= m.banca ? '+' : ''}{brl(bancaReal - m.banca)})</div>}
                            </div>
                            <button onClick={() => B.removerMarco(m.dia)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text3)', padding: 4 }}><X size={14} /></button>
                          </div>
                        );
                      })}
                    </div>
        ...
        {B.screen === 'estatisticas' && B.challenge && (() => {
          ...
          if (!stats) return <div className="screen active page" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: 300 }}><p style={{ color: 'var(--text3)' }}>Nenhum dia registrado ainda.</p></div>;
          ...
          return (
            <div className="screen active page">
              ...
              {stats.streakTipo && (
                <div style={{ background: stats.streakTipo === 'ganhou' ? 'rgba(34,197,94,0.08)' : 'rgba(239,68,68,0.08)', border: `1px solid ${stats.streakTipo === 'ganhou' ? 'rgba(34,197,94,0.25)' : 'rgba(239,68,68,0.25)'}`, borderRadius: 12, padding: '12px 18px', marginBottom: 20, display: 'flex', alignItems: 'center', gap: 12 }}>
                  {stats.streakTipo === 'ganhou' ? <Flame size={18} color="var(--green)" /> : <AlertCircle size={18} color="var(--red)" />}
                  <div>
                    <div style={{ fontWeight: 700, fontSize: 14, color: stats.streakTipo === 'ganhou' ? 'var(--green)' : 'var(--red)' }}>Sequência atual: {stats.streakAtual} {stats.streakTipo === 'ganhou' ? 'vitória(s)' : 'derrota(s)'}</div>
                    <div style={{ fontSize: 12, color: 'var(--text3)' }}>{stats.streakTipo === 'ganhou' ? 'Continue assim!' : 'Considere revisar a estratégia.'}</div>
                  </div>
                </div>
              )}
              <div className="two-col-grid" style={{ marginBottom: 20 }}>
                    ...
                    <div style={{ height: 180 }}>
                      <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={barData} barSize={36}>
                          ...
                          <Tooltip contentStyle={{ backgroundColor: '#1a1e28', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 8 }} formatter={(v, _, { payload }) => [`${v}% (${payload.total} dias)`, 'Taxa']} />
                          <Bar dataKey="taxa" radius={[6, 6, 0, 0]}>{barData.map((e) => <Cell key={e.name} fill={e.cor} />)}</Bar>
                        </BarChart>
                      </ResponsiveContainer>
                    </div>
                  </div>
                ...
                <div className="card">
                  ...
                  {stats.melhorDia && <div style={{ background: 'rgba(34,197,94,0.07)', borderRadius: 10, padding: '12px 14px', marginBottom: 10 }}><div style={{ fontSize: 11, color: 'var(--green)', fontWeight: 700, textTransform: 'uppercase', marginBottom: 4 }}>Melhor dia — Dia {stats.melhorDia.dia}</div><div style={{ fontSize: 18, fontWeight: 800, color: 'var(--green)', fontFamily: 'monospace' }}>+{brl(stats.melhorDia.lucro)}</div></div>}
                  {stats.piorDia   && <div style={{ background: 'rgba(239,68,68,0.07)',  borderRadius: 10, padding: '12px 14px' }}><div style={{ fontSize: 11, color: 'var(--red)', fontWeight: 700, textTransform: 'uppercase', marginBottom: 4 }}>Pior dia — Dia {stats.piorDia.dia}</div><div style={{ fontSize: 18, fontWeight: 800, color: 'var(--red)', fontFamily: 'monospace' }}>{brl(stats.piorDia.lucro)}</div></div>}
                </div>
              </div>
              {tagStats.length > 0 && (
                <div className="card" style={{ marginBottom: 20 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16 }}><Tag size={14} color="var(--accent2)" /><div className="section-title" style={{ fontSize: 14 }}>Taxa de acerto por mercado</div></div>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                    {tagStats.sort((a, b) => b.taxa - a.taxa).map((tag) => (
                      <div key={tag.id}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 5 }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}><span style={{ width: 8, height: 8, borderRadius: '50%', background: tag.color, display: 'inline-block' }} /><span style={{ fontSize: 13, fontWeight: 600, color: 'var(--text)' }}>{tag.label}</span><span style={{ fontSize: 11, color: 'var(--text3)' }}>{tag.ganhos}/{tag.total}</span></div>
                          <span style={{ fontSize: 13, fontWeight: 700, color: tag.taxa >= 60 ? 'var(--green)' : tag.taxa >= 40 ? 'var(--amber)' : 'var(--red)' }}>{tag.taxa.toFixed(0)}%</span>
                        </div>
                        <div style={{ height: 5, background: 'rgba(255,255,255,0.06)', borderRadius: 4, overflow: 'hidden' }}><div style={{ height: '100%', borderRadius: 4, width: `${tag.taxa}%`, background: tag.taxa >= 60 ? 'var(--green)' : tag.taxa >= 40 ? 'var(--amber)' : 'var(--red)', transition: 'width .5s' }} /></div>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <div className="two-col-grid">
                <div>
                  <div className="card" style={{ marginBottom: 16 }}>
                    ...
                    <div className="input-group"><label>Banca inicial (R$)</label><input type="number" value={B.calcBancaI} onChange={(e) => B.setCalcBancaI(e.target.value)} /></div>
                    <div className="input-group"><label>Meta final (R$)</label><input type="number" value={B.calcMeta} onChange={(e) => B.setCalcMeta(e.target.value)} /></div>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                      <div className="input-group"><label>Dias</label><input type="number" min={1} value={B.calcDias} onChange={(e) => B.setCalcDias(e.target.value)} /></div>
                      <div className="input-group"><label>Entradas</label><input type="number" min={1} max={6} value={B.calcNEntradas} onChange={(e) => B.setCalcNEntradas(e.target.value)} /></div>
                    </div>
                    {oddDiaria ? (
                      <div style={{ background: 'rgba(79,110,247,0.1)', border: '1px solid rgba(79,110,247,0.25)', borderRadius: 14, padding: '18px 20px', marginTop: 8 }}>
                        ...
                        {oddAtual && diasNec !== null && (
                          <div style={{ marginTop: 10, padding: '8px 12px', background: 'rgba(255,255,255,0.05)', borderRadius: 8, fontSize: 12, color: 'var(--text2)' }}>
                            Com odd {oddAtual}x/dia precisaria de <strong style={{ color: diasNec <= parseInt(B.calcDias) ? 'var(--green)' : 'var(--amber)' }}>{diasNec} dias</strong>{diasNec <= parseInt(B.calcDias) ? ' — viável!' : ` — ${diasNec - parseInt(B.calcDias)} a mais.`}
                          </div>
                        )}
                      </div>
                    ) : (
                      <div style={{ background: 'rgba(239,68,68,0.08)', borderRadius: 12, padding: '14px 16px', marginTop: 8, fontSize: 13, color: 'var(--red)' }}>A meta deve ser maior que a banca inicial.</div>
                    )}
                  </div>
                  {dutching.length > 0 && (
                    <div className="card">
                      ...
                      {dutching.map((d, i) => (
                        <div key={i} className="split-card" style={{ borderLeft: `3px solid ${'#4f6ef7,#7c5cfa,#a78bfa,#c4b5fd,#ddd6fe,#ede9fe'.split(',')[i] || '#4f6ef7'}`, marginBottom: 8 }}>
                          <div><div className="split-num">ENTRADA {i + 1}</div><div style={{ fontWeight: 700, fontSize: 15, marginTop: 3, fontFamily: 'monospace' }}>{brl(d.apostado)} <span style={{ fontSize: 12, color: 'var(--text3)', fontWeight: 400 }}>@ {d.odd.toFixed(3)}x</span></div></div>
                          <div style={{ textAlign: 'right' }}><div style={{ fontSize: 12, color: 'var(--green)', fontWeight: 700 }}>+{brl(d.lucro)}</div><div style={{ fontSize: 14, fontWeight: 700, fontFamily: 'monospace' }}>{brl(d.retorno)}</div></div>
                        </div>
                      ))}
                    </div>
                  )}
                  ...
                  {oddDiaria && (
                    <div className="card">
                      ...
                      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
                        <thead><tr style={{ borderBottom: '1px solid var(--border)' }}>{['Dia','Banca projetada','Lucro acum.'].map((h) => <th key={h} style={{ textAlign: h==='Dia'?'left':'right', padding: '6px 0', color: 'var(--text3)', fontWeight: 600, fontSize: 11, textTransform: 'uppercase' }}>{h}</th>)}</tr></thead>
                        <tbody>
                          {(() => {
                            const total = parseInt(B.calcDias)||1, bancaI = parseFloat(B.calcBancaI)||0;
                            return [...new Set([Math.max(1,Math.ceil(total*.25)),Math.max(1,Math.ceil(total*.5)),Math.max(1,Math.ceil(total*.75)),total])].map((d) => {
                              const banca = parseFloat((bancaI*Math.pow(oddDiaria,d)).toFixed(2)), lucro = parseFloat((banca-bancaI).toFixed(2));
                              return <tr key={d} style={{ borderBottom: '1px solid var(--border)' }}><td style={{ padding: '10px 0', fontWeight: 700, color: 'var(--accent2)' }}>Dia {d}</td><td style={{ padding: '10px 0', textAlign: 'right', fontFamily: 'monospace', fontWeight: 700 }}>{brl(banca)}</td><td style={{ padding: '10px 0', textAlign: 'right', color: lucro>=0?'var(--green)':'var(--red)', fontWeight: 700 }}>{lucro>=0?'+':''}{brl(lucro)}</td></tr>;
                            });
                          })()}
                        </tbody>
                      </table>
          ...
          <div className="screen active page">
            <div className="modo-selector">
              {[
                ...
                { key: 'misto', label: 'Misto', Icon: GitMerge,   desc: '1+2 grupo' },
              ].map(({ key, label, Icon, desc }) => (
                <button key={key} onClick={() => B.handleModoChange(key)} className={`modo-btn ${B.modo === key ? 'active' : ''}`}>
                  <div className="modo-btn-title"><Icon size={14} /> {label}</div>
                  <div className="modo-btn-desc">{desc}</div>
                </button>
              ))}
            </div>
            ...
            {B.modo === 'misto' && (
              <div style={{ background: 'rgba(245,158,11,0.07)', border: '1px solid rgba(245,158,11,0.2)', borderRadius: 12, padding: '14px 18px', marginBottom: 24, display: 'flex', alignItems: 'center', gap: 20, flexWrap: 'wrap' }}>
                <div style={{ flex: 1, minWidth: 180 }}>
                  ...
                  <input type="range" min={1} max={Math.max(1, B.entradas.length - 1)} value={B.grupoSize} onChange={(e) => { B.setGrupoSize(parseInt(e.target.value)); B.setEntradas(B.entradas.map((e) => ({ ...e, status: 'pendente' }))); }} style={{ width: '100%', accentColor: 'var(--amber)' }} />
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, color: 'var(--text3)', marginTop: 4 }}><span>1 no grupo</span><span>{Math.max(1, B.entradas.length - 1)} no grupo</span></div>
                </div>
                <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                  {B.entradas.map((_, i) => (
                    <div key={i} onClick={() => { const n = i+1<=B.entradas.length-1?i+1:i; if(n!==B.grupoSize){B.setGrupoSize(n);B.setEntradas(B.entradas.map((e)=>({...e,status:'pendente'})));} }} style={{ width: 32, height: 32, borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 700, cursor: 'pointer', background: i<B.grupoSize?'rgba(245,158,11,0.2)':'rgba(79,110,247,0.15)', color: i<B.grupoSize?'var(--amber)':'var(--accent2)', border: `1px solid ${i<B.grupoSize?'rgba(245,158,11,0.3)':'rgba(79,110,247,0.3)'}` }}>{i+1}</div>
                  ))}
                </div>
              </div>
            )}
            ...
            <div className="dia-grid">
              ...
              <div>
                <div className="card">
                  ...
                  <div style={{ background: 'rgba(79,110,247,0.07)', borderRadius: 10, padding: '10px 14px', marginBottom: 18, fontSize: 12, color: 'var(--text2)', lineHeight: 1.6 }}>
                    {B.modo === 'soros' && 'Retorno de cada entrada vira a aposta da próxima.'}
                    ...
                    {B.passos.map((passo, i) => {
                      ...
                      return (
                        <React.Fragment key={passo.id}>
                          {primLivre && <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--accent2)', letterSpacing: 0.6, textTransform: 'uppercase', borderTop: '1px solid var(--border)', paddingTop: 14, marginTop: 4 }}>▸ LIVRE — SOROS SOBRE RETORNO DO GRUPO</div>}
                              ...
                              <div>
                                ...
                                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 6 }}>
                                  ...
                                  <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
                                    <input type="number" step="0.01" value={passo.odd} disabled={passo.status!=='pendente'||bloqueada} style={{ width: 80, padding: '6px 10px', fontSize: 14, borderColor: B.errosOdd[i]?'var(--red)':undefined }} onChange={(e) => B.atualizarOdd(i, e.target.value)} />
                                ...
                                {passo.status==='pendente'&&!bloqueada && (
                                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 8 }}>
                                    ...
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                                      <input type="number" step="0.01" min="0.01" placeholder={brl(passo.apostado).replace('R$\u00a0','')} value={B.entradas[i]?.apostadoReal??''} style={{ width: 90, padding: '5px 9px', fontSize: 13, fontFamily: 'monospace', borderColor: B.entradas[i]?.apostadoReal?'rgba(167,139,250,0.5)':undefined }} onChange={(e) => B.atualizarApostadoReal(i, e.target.value)} />
                                      {B.entradas[i]?.apostadoReal && <span style={{ fontSize: 10, color: '#a78bfa' }}>retorno: {brl(parseFloat(B.entradas[i].apostadoReal)*(parseFloat(passo.odd)||1))}</span>}
                                    </div>
                                  </div>
                                )}
                              </div>
                              <div style={{ textAlign: 'right' }}>
                                <div style={{ fontSize: 11, color: 'var(--text3)' }}>{B.entradas[i]?.apostadoReal?'Valor real':'Sugerido'}</div>
                                <div className="split-val" style={{ color: B.entradas[i]?.apostadoReal?'#a78bfa':'var(--text)' }}>{brl(passo.apostado)}</div>
                                <div style={{ fontSize: 12, color: 'var(--text2)' }}>Retorno: <strong style={{ color: passo.retorno>0?'#fff':'var(--text3)' }}>{brl(passo.retorno>0?passo.retorno:(passo.apostado*(parseFloat(passo.odd)||1)))}</strong></div>
                                {B.entradas[i]?.apostadoReal&&parseFloat(B.entradas[i].apostadoReal)!==passo.apostadoCalc && <div style={{ fontSize: 10, color: 'var(--text3)', marginTop: 2 }}>calc: {brl(passo.apostadoCalc??passo.apostado)}</div>}
                              </div>
                            </div>
                            {passo.status==='pendente'&&!bloqueada && (
                  ...
                  {B.modo === 'media' ? (() => {
                    ...
                    const atingiu=ganhouVal>=meta, impossivel=maxPoss<meta&&pendentes.length===0;
                    return (
                      <div style={{ marginTop:20,background:'var(--bg3)',borderRadius:14,overflow:'hidden' }}>
                        <div style={{ height:6,background:'rgba(255,255,255,0.06)' }}><div style={{ height:'100%',width:`${Math.max(1,pct)}%`,background:atingiu?'var(--green)':'var(--accent)',transition:'width .4s ease',borderRadius:4 }} /></div>
                        <div style={{ padding:'16px 18px' }}>
                          ...
                          <div style={{ borderTop:'1px dashed rgba(255,255,255,0.08)',paddingTop:14 }}>
                            <div style={{ display:'flex',justifyContent:'space-between',alignItems:'flex-end' }}>
                              <div>
                                <div style={{ fontSize:11,fontWeight:700,color:'var(--text3)',letterSpacing:.5,textTransform:'uppercase',marginBottom:5 }}>{atingiu?'Meta atingida!':pendentes.length>0?'Acumulado':'Resultado final'}</div>
                                <div style={{ fontSize:28,fontWeight:800,fontFamily:'monospace',color:atingiu?'var(--green)':'var(--text)' }}>{brl(ganhouVal)}</div>
                                {perdeuVal>0 && <div style={{ fontSize:11,color:'var(--red)',marginTop:3 }}>perdido: {brl(perdeuVal)}</div>}
                              </div>
                              <div style={{ textAlign:'right' }}>
                                {atingiu?(
                                  <div style={{ background:'rgba(34,197,94,0.12)',border:'1px solid rgba(34,197,94,0.3)',borderRadius:10,padding:'10px 16px' }}>
                                    ...
                                    <div style={{ fontSize:11,color:'rgba(34,197,94,.6)',marginTop:2 }}>+{brl(ganhouVal-banca)}</div>
                                  </div>
                                ):impossivel?(
                                  <div style={{ background:'rgba(239,68,68,0.1)',border:'1px solid rgba(239,68,68,0.25)',borderRadius:10,padding:'10px 16px' }}>
                                    ...
                                    <div style={{ fontSize:11,color:'rgba(239,68,68,.6)',marginTop:2 }}>restaram {brl(maxPoss)}</div>
                                  </div>
                                ):(
                                  <div style={{ background:'rgba(79,110,247,0.1)',border:'1px solid rgba(79,110,247,0.25)',borderRadius:10,padding:'10px 16px' }}>
                                    ...
                                    {pendentes.length>0 && <div style={{ fontSize:10,color:'var(--text3)',marginTop:3 }}>{pendentes.length} pendente(s)</div>}
                                  </div>
                                )}
                              </div>
                            </div>
                    ...
                    <div className="resultado-final" style={{ marginTop:24,marginBottom:0 }}>
                      <div><div style={{ fontSize:11,fontWeight:700,color:'var(--text3)',textTransform:'uppercase',marginBottom:6 }}>Se acertar todas</div><div className="val">{brl(B.retornoProjetado)}</div></div>
                      <div style={{ textAlign:'right',fontSize:13,fontWeight:600,color:B.retornoProjetado>=B.diaAtual.meta?'var(--green)':'var(--red)' }}>{B.retornoProjetado>=B.diaAtual.meta?'✓ Meta atingida':`Faltam ${brl(B.diaAtual.meta-B.retornoProjetado)}`}</div>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div style={{ display:'flex',flexDirection:'column',gap:16 }}>
                <div className="card" style={{ border:B.diaAtual.aporte>0?'1px solid rgba(34,197,94,0.3)':'1px solid var(--border)',background:B.diaAtual.aporte>0?'rgba(34,197,94,0.04)':'var(--bg2)' }}>
                  <div style={{ fontSize:11,fontWeight:700,color:'var(--text3)',letterSpacing:.6,textTransform:'uppercase',marginBottom:14 }}>Banca do dia</div>
                  <div style={{ display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:10 }}><span style={{ fontSize:13,color:'var(--text3)' }}>Banca base</span><span style={{ fontWeight:700,fontFamily:'monospace',color:'var(--text)' }}>{brl(B.diaAtual.bancaInicio)}</span></div>
                  <div style={{ marginBottom:12 }}>
                    <label style={{ fontSize:11,marginBottom:6 }}>Adicionar dinheiro extra (aporte)</label>
                    <div style={{ display:'flex',gap:8,alignItems:'center' }}>
                      <input type="number" min={0} step="0.01" placeholder="0,00" value={B.diaAtual.aporte||''} onChange={(e) => B.salvarAporte(e.target.value)} style={{ flex:1,padding:'10px 12px',fontSize:15,fontFamily:'monospace' }} />
                      {B.diaAtual.aporte>0 && <button onClick={() => B.salvarAporte(0)} style={{ background:'none',border:'none',cursor:'pointer',color:'var(--text3)',padding:6 }}><X size={16} /></button>}
                    </div>
                  </div>
                  <div style={{ borderTop:'1px dashed var(--border)',paddingTop:12,marginTop:4 }}>
                    <div style={{ display:'flex',justifyContent:'space-between',alignItems:'center' }}><span style={{ fontSize:14,fontWeight:700,color:'var(--text)' }}>Banca efetiva</span><span style={{ fontSize:22,fontWeight:800,fontFamily:'monospace',color:B.diaAtual.aporte>0?'var(--green)':'var(--text)' }}>{brl(B.bancaEfetiva)}</span></div>
                ...
                <div className="card">
                  ...
                  <div style={{ marginBottom:14 }}>
                    <div style={{ fontSize:11,fontWeight:700,color:'var(--text3)',letterSpacing:.5,textTransform:'uppercase',marginBottom:8 }}>Mercados apostados</div>
                    <div style={{ display:'flex',flexWrap:'wrap',gap:6 }}>
                      {TAGS_DISPONIVEIS.map((tag) => {
                        const ativa=(B.diaAtual.tags||[]).includes(tag.id);
                        return <button key={tag.id} onClick={() => B.toggleTag(tag.id)} style={{ padding:'4px 10px',borderRadius:20,fontSize:11,fontWeight:600,cursor:'pointer',border:`1px solid ${ativa?tag.color:'var(--border2)'}`,background:ativa?`${tag.color}22`:'transparent',color:ativa?tag.color:'var(--text3)',transition:'all .15s',fontFamily:'inherit' }}>{tag.label}</button>;
                      })}
                    </div>
                  </div>
                ...
                <div className="card">
                  ...
                  <div style={{ display:'flex',flexDirection:'column',gap:10 }}>
                    {[
                      ...
                      ...(B.diaAtual.oddSugerida?[{ label:'Odd sugerida', value:`${B.diaAtual.oddSugerida.toFixed(3)}x`, color:'var(--amber)' }]:[]),
                    ].map(({ label, value, color }) => (
                      <div key={label} style={{ display:'flex',justifyContent:'space-between',alignItems:'center',fontSize:13 }}><span style={{ color:'var(--text3)' }}>{label}</span><span style={{ fontWeight:700,color,fontFamily:'monospace' }}>{value}</span></div>
                    ))}
                  </div>
                </div>
                ...
                {B.modo==='misto' && (
                  <div className="card" style={{ background:'rgba(245,158,11,0.05)',borderColor:'rgba(245,158,11,0.2)' }}>
                    ...
                    {(() => { const R=B.passos[0]?.retorno||0; return <div style={{ fontSize:12,color:'var(--text2)',lineHeight:2 }}><div>Todas ganham: <strong style={{ color:'var(--green)' }}>{brl(R*B.grupoSize)}</strong></div>{B.grupoSize>1&&<div>{B.grupoSize-1}/{B.grupoSize}: <strong style={{ color:'var(--amber)' }}>{brl(R*(B.grupoSize-1))}</strong></div>}<div>Só 1: <strong style={{ color:'var(--amber)' }}>{brl(R)}</strong></div><div>Todas perdem: <strong style={{ color:'var(--red)' }}>R$ 0,00</strong></div></div>; })()}
                  </div>
                )}
                {B.modo==='media' && (
                  <div className="card" style={{ background:'rgba(79,110,247,0.05)',borderColor:'rgba(79,110,247,0.2)' }}>
                    ...
                    {(() => { const R=B.passos[0]?.retorno||0,n=B.passos.length; return <div style={{ fontSize:12,color:'var(--text2)',lineHeight:2 }}>{[n,n-1,Math.ceil(n/2),1,0].filter((v,i,a)=>a.indexOf(v)===i&&v>=0).map((k)=><div key={k}>{k===n?'Todas ganham':k===0?'Todas perdem':`${k}/${n} ganham`}: <strong style={{ color:k>=Math.ceil(n/2)?'var(--green)':k>0?'var(--amber)':'var(--red)' }}>{brl(k*R)}</strong></div>)}</div>; })()}
                  </div>
                )}
              </div>
            </div>
